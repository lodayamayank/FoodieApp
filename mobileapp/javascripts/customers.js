...
$(document).on('click', "#addButton", function () {
    $("#recordId").val("");
    resetFields("customersModal");
    $("#customersModal").modal("open");
})
...
function saveUpdatecustomers() {
    ...
    if (recordId != "") {
        objParams.url = "/updateAjaxcustomers";
        objParams.recordId = recordId;
    } else {
        objParams.url = "/saveAjaxcustomers"
    }
...
$(document).on('click', '.deleteRecord', function () {
    ...
    swal({
        ...
        if (willDelete) {
            ...
            objParams.url = "/deleteAjaxcustomers";
            sendAjaxRequest(objParams, function (response) {
                if (response.status == 1) {
                    M.toast({ html: response.err, classes: 'rounded' });
                } else if (response.status == 0) {
...
$(document).on('click', '.editRecord', function () {
    ...
    var objParams = {};
    objParams.recordId = recordId;
    objParams.url = "/getcustomersRecordById";
    sendAjaxRequest(objParams, function (response) {
        if (response.status == 1) {
            M.toast({ html: response.err, classes: 'rounded' });
        } else if (response.status == 0) {
