...
$(document).on('click', "#addButton", function () {
    $("#recordId").val("");
    resetFields("brandsModal");
    $("#brandsModal").modal("open");
})
...
function saveUpdatebrands() {
    ...
    if (recordId != "") {
        objParams.url = "/updateAjaxbrands";
        objParams.recordId = recordId;
    } else {
        objParams.url = "/saveAjaxbrands"
    }
...
$(document).on('click', '.deleteRecord', function () {
    ...
    swal({
        ...
        if (willDelete) {
            ...
            objParams.url = "/deleteAjaxbrands";
            sendAjaxRequest(objParams, function (response) {
                if (response.status == 1) {
                    M.toast({ html: response.err, classes: 'rounded' });
                } else if (response.status == 0) {
                    swal("Brands has been deleted successfully", {
                        icon: "success",
                    });
                    getbrandsList();
                } else {
...
$(document).on('click', '.editRecord', function () {
    ...
    var objParams = {};
    objParams.recordId = recordId;
    objParams.url = "/getbrandsRecordById";
    sendAjaxRequest(objParams, function (response) {
        if (response.status == 1) {
            M.toast({ html: response.err, classes: 'rounded' });
        } else if (response.status == 0) {
