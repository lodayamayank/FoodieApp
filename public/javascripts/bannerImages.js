...
$(document).on("click", "#save", function () {
    saveUpdateBannerImages();
});
$(document).on('click', "#addButton", function () {
    $("#recordId").val("");
    resetFields("bannerImagesModal");
    $("#bannerImagesModal").modal("open");
})
function getBannerImagesList() {
    $("#grid").kendoGrid({
        dataSource: {
            // type: "json",
            transport: {
...
function saveUpdateBannerImages() {
    ...
    if (recordId != "") {
        objParams.url = "/updateAjaxBannerImages";
        objParams.recordId = recordId;
    } else {
        objParams.url = "/saveAjaxBannerImages"
    }
...
$(document).on('click', '.deleteRecord', function () {
    ...
    swal({
        ...
        if (willDelete) {
            ...
            objParams.url = "/deleteAjaxBannerImages";
            sendAjaxRequest(objParams, function (response) {
                if (response.status == 1) {
                    M.toast({ html: response.err, classes: 'rounded' });
                } else if (response.status == 0) {
                    swal("Bannerimages has been deleted successfully", {
                        icon: "success",
                    });
                    getBannerImagesList();
                } else {
...
$(document).on('click', '.editRecord', function () {
    ...
    var objParams = {};
    objParams.recordId = recordId;
    objParams.url = "/getBannerImagesRecordById";
    sendAjaxRequest(objParams, function (response) {
        if (response.status == 1) {
            M.toast({ html: response.err, classes: 'rounded' });
        } else if (response.status == 0) {
