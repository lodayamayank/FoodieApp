...
$(document).on("click", "#save", function () {
    saveUpdatemanufacturers();
});
$(document).on('click', "#addButton", function () {
    $("#recordId").val("");
    resetFields("manufacturersModal");
    $("#manufacturersModal").modal("open");
})
...
function saveUpdatemanufacturers() {
    ...
    if (recordId != "") {
        objParams.url = "/updateAjaxmanufacturers";
        objParams.recordId = recordId;
    } else {
        objParams.url = "/saveAjaxmanufacturers"
    }
    ...
    sendAjaxRequest(objParams, function (response) {
        if (response.status == 1) {
            ...
            $("#save").prop("disable", false);
            $("#manufacturersModal").modal("close");
            getmanufacturersList();
        } else {
            $("#save").prop("disable", false);
...
$(document).on('click', '.deleteRecord', function () {
    ...
    swal({
        ...
        if (willDelete) {
            ...
            objParams.url = "/deleteAjaxmanufacturers";
            sendAjaxRequest(objParams, function (response) {
                if (response.status == 1) {
                    M.toast({ html: response.err, classes: 'rounded' });
                } else if (response.status == 0) {
                    swal("Coupons has been deleted successfully", {
                        icon: "success",
                    });
                    getmanufacturersList();
                } else {
...
$(document).on('click', '.editRecord', function () {
    ...
    var objParams = {};
    objParams.recordId = recordId;
    objParams.url = "/getmanufacturersRecordById";
    sendAjaxRequest(objParams, function (response) {
        if (response.status == 1) {
            M.toast({ html: response.err, classes: 'rounded' });
        } else if (response.status == 0) {
