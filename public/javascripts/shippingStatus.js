...
$(document).on("click", "#save", function () {
    saveUpdateShippingStatus();
});
$(document).on('click', "#addButton", function () {
    $("#recordId").val("");
    resetFields("shippingStatusModal");
    $("#shippingStatusModal").modal("open");
})
function getShippingStatusList() {
    $("#grid").kendoGrid({
        dataSource: {
            // type: "json",
            transport: {
                read: {
                    url: "/getShippingStatus",
                    dataType: "json",
                    type: "POST",
                },
...
function saveUpdateShippingStatus() {
    ...
    if (recordId != "") {
        objParams.url = "/updateAjaxShippingStatus";
        objParams.recordId = recordId;
    } else {
        objParams.url = "/saveAjaxShippingStatus"
    }
    ...
    sendAjaxRequest(objParams, function (response) {
        if (response.status == 1) {
            ...
            $("#save").prop("disable", false);
            $("#shippingStatusModal").modal("close");
            getShippingStatusList();
        } else {
            $("#save").prop("disable", false);
...
$(document).on('click', '.deleteRecord', function () {
    ...
    swal({
        ...
        if (willDelete) {
            ...
            objParams.url = "/deleteAjaxShippingStatus";
            sendAjaxRequest(objParams, function (response) {
                if (response.status == 1) {
                    M.toast({ html: response.err, classes: 'rounded' });
                } else if (response.status == 0) {
                    swal("ShippingStatu has been deleted successfully", {
                        icon: "success",
                    });
                    getShippingStatusList();
                } else {
...
$(document).on('click', '.editRecord', function () {
    ...
    var objParams = {};
    objParams.recordId = recordId;
    objParams.url = "/getShippingStatuRecordById";
    sendAjaxRequest(objParams, function (response) {
        if (response.status == 1) {
            M.toast({ html: response.err, classes: 'rounded' });
        } else if (response.status == 0) {
