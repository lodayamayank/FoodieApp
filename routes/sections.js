...
module.exports = function (app, gConfig) {
    ...
    gConfig.verifyToken(token, function (responseToken) {
      if (responseToken != false) {
        gConfig.getUserMenu(responseToken, token, function (menu) {
          ...
          resObj.title = "Sections";
          resObj.menu = menu;
          resObj.token = token;
          res.render('sections', resObj);
        })
  ...
  getSectionsList = function (req, res) {
    var responseJSON = {};
    try {
      ...
      gConfig.verifyToken(token, function (responseToken) {
        if (responseToken != false) {
          ...
          gConfig.SectionsSchema.find(condition).sort({ "order": 1 }).exec(function (errSchema, resSchema) {
            if (errSchema) {
              ...
              var index = 0;
              var totalRecords = 0;
              gConfig.async.eachSeries(resSchema, function (section, sectionCallback) {
                index++;
                var json = {};
                json.index = index;
                json._id = section._id;
                json.name = section.name;
                json.order = section.order;
                json.createdOn = gConfig.moment(section.createdOn).format("DD/MM/YYYY");
                json.updatedOn = gConfig.moment(section.updatedOn).format("DD/MM/YYYY");
                arrRecords.push(json);
                sectionCallback();

              }, function () {
                gConfig.SectionsSchema.count(condition).exec(function (err, count) {
                  totalRecords = count;
  ...
  getSectionRecordById = function (req, res) {
    var responseJSON = {};
    try {
      var token = req.cookies.token;
      gConfig.verifyToken(token, function (responseToken) {
        if (responseToken != false) {
          ...
          gConfig.SectionsSchema.findOne(condition).sort({ "order": 1 }).exec(function (errSchema, resSchema) {
            if (errSchema) {
              responseJSON.status = 1;
    ...
    try {
      ...
      gConfig.verifyToken(token, function (responseToken) {
        if (responseToken != false) {
          ...
          gConfig.SectionsSchema.findOne(condition).exec(function (errSchema, resSchema) {
            if (errSchema) {
              responseJSON.status = 1;
              responseJSON.err = '';
              return res.json(responseJSON);
              ...
              var resSaveCommon = new gConfig.SectionsSchema({});
    ...
    try {
      ...
      gConfig.verifyToken(token, function (responseToken) {
        if (responseToken != false) {
          ...
          gConfig.SectionsSchema.find(condition).exec(function (errSchema, resSchema) {
            if (errSchema) {
              responseJSON.status = 1;
              responseJSON.err = '';
              return res.json(responseJSON);
  ...
  app.get('/sections/:tokenKey', getSection);
  app.post('/getSections', getSectionsList);
  app.post('/getSectionRecordById', getSectionRecordById);
  app.post('/saveAjaxSections', saveSection);
  app.post('/updateAjaxSections', updateSection);
  app.post('/deleteAjaxSections', deleteSection);
}