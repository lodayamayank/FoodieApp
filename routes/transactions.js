...
module.exports = function (app, gConfig) {
    ...
    gConfig.verifyToken(token, function (responseToken) {
      if (responseToken != false) {
        gConfig.getUserMenu(responseToken, token, function (menu) {
          ...
          resObj.title = "transactions";
          resObj.menu = menu;
          resObj.token = token;
          res.render('transactions', resObj);
        })
  ...
  gettransactionsList = function (req, res) {
    var responseJSON = {};
    try {
      ...
      gConfig.verifyToken(token, function (responseToken) {
        if (responseToken != false) {
          ...
          gConfig.transactionsSchema.find(condition).sort({ "order": 1 }).exec(function (errSchema, resSchema) {
            if (errSchema) {
              ...
              var index = 0;
              var totalRecords = 0;
              gConfig.async.eachSeries(resSchema, function (transactions, transactionsCallback) {
                index++;
                var json = {};
                json.index = index;
                json._id = transactions._id;
                json.name = transactions.name;
                json.order = transactions.order;
                json.createdOn = gConfig.moment(transactions.createdOn).format("DD/MM/YYYY");
                json.updatedOn = gConfig.moment(transactions.updatedOn).format("DD/MM/YYYY");
                arrRecords.push(json);
                transactionsCallback();

              }, function () {
                gConfig.transactionsSchema.count(condition).exec(function (err, count) {
                  totalRecords = count;
  ...
  gettransactionsRecordById = function (req, res) {
    var responseJSON = {};
    try {
      var token = req.cookies.token;
      gConfig.verifyToken(token, function (responseToken) {
        if (responseToken != false) {
          ...
          gConfig.transactionsSchema.findOne(condition).sort({ "order": 1 }).exec(function (errSchema, resSchema) {
            if (errSchema) {
              responseJSON.status = 1;
  ...
  savetransactions = function (req, res) {
    var responseJSON = {};
    try {
      var token = req.cookies.token;
      gConfig.verifyToken(token, function (responseToken) {
        if (responseToken != false) {
          ...
          gConfig.transactionsSchema.findOne(condition).exec(function (errSchema, resSchema) {
            if (errSchema) {
              responseJSON.status = 1;
              responseJSON.err = '';
              return res.json(responseJSON);
              ...
              var resSaveCommon = new gConfig.transactionsSchema({});
    ...
    try {
      ...
      gConfig.verifyToken(token, function (responseToken) {
        if (responseToken != false) {
          ...
          gConfig.transactionsSchema.find(condition).exec(function (errSchema, resSchema) {
            if (errSchema) {
              responseJSON.status = 1;
              responseJSON.err = '';
              return res.json(responseJSON);
  ...
  app.get('/transactions/:tokenKey', gettransactions);
  app.post('/gettransactions', gettransactionsList);
  app.post('/gettransactionsRecordById', gettransactionsRecordById);
  app.post('/saveAjaxtransactions', savetransactions);
  app.post('/updateAjaxtransactions', updatetransactions);
  app.post('/deleteAjaxtransactions', deletetransactions);
}