...
module.exports = function (app, gConfig) {
    ...
    gConfig.verifyToken(token, function (responseToken) {
      if (responseToken != false) {
        gConfig.getUserMenu(responseToken, token, function (menu) {
          ...
          resObj.title = "Sub Category";
          resObj.menu = menu;
          resObj.token = token;
          res.render('subCategory', resObj);
        })
    ...
    try {
      ...
      gConfig.verifyToken(token, function (responseToken) {
        if (responseToken != false) {
          ...
          gConfig.SubcategorySchema.find(condition).populate("categoryId").populate("sectionId").sort({ "order": 1 }).exec(function (errSchema, resSchema) {
            if (errSchema) {
              ...
              var index = 0;
              var totalRecords = 0;
              gConfig.async.eachSeries(resSchema, function (subcategory, subcategoryCallback) {
                index++;
                var json = {};
                json.index = index;
                json._id = subcategory._id;
                json.name = subcategory.name;
                json.categoryName = subcategory.categoryId.name;
                json.sectionName = subcategory.sectionId.name;
                json.createdOn = gConfig.moment(subcategory.createdOn).format("DD/MM/YYYY");
                json.updatedOn = gConfig.moment(subcategory.updatedOn).format("DD/MM/YYYY");
                arrRecords.push(json);
                subcategoryCallback();

              }, function () {
                gConfig.SubcategorySchema.count(condition).exec(function (err, count) {
                  totalRecords = count;
  ...
  getSubCategoryRecordById = function (req, res) {
    var responseJSON = {};
    try {
      var token = req.cookies.token;
      gConfig.verifyToken(token, function (responseToken) {
        if (responseToken != false) {
          ...
          gConfig.SubcategorySchema.findOne(condition).sort({ "order": 1 }).exec(function (errSchema, resSchema) {
            if (errSchema) {
              responseJSON.status = 1;
  ...
  saveSubCategory = function (req, res) {
    var responseJSON = {};
    try {
      var token = req.cookies.token;
      gConfig.verifyToken(token, function (responseToken) {
        if (responseToken != false) {
          ...
          condition.sectionId = req.body.section;
          condition.categoryId = req.body.category;
          gConfig.SubcategorySchema.findOne(condition).exec(function (errSchema, resSchema) {
            if (errSchema) {
              responseJSON.status = 1;
              responseJSON.err = '';
              return res.json(responseJSON);
              ...
              responseJSON.err = 'Sub category already present.';
              return res.json(responseJSON);
            } else {
              var resSaveCommon = new gConfig.SubcategorySchema({});
              ...
              if (req.body.section != '' && req.body.section != undefined) {
                resSaveCommon.sectionId = req.body.section;
              }
              if (req.body.category != '' && req.body.category != undefined) {
                resSaveCommon.categoryId = req.body.category;
              }
    ...
    try {
      ...
      gConfig.verifyToken(token, function (responseToken) {
        if (responseToken != false) {
          ...
          gConfig.SubcategorySchema.find(condition).exec(function (errSchema, resSchema) {
            if (errSchema) {
              responseJSON.status = 1;
              responseJSON.err = '';
              return res.json(responseJSON);
  ...
  app.get('/subcategories/:tokenKey', getSubCategory);
  app.post('/getSubCategory', getSubCategoryList);
  app.post('/getSubCategoryRecordById', getSubCategoryRecordById);
  app.post('/saveAjaxSubCategory', saveSubCategory);
  app.post('/updateAjaxSubCategory', updateSubCategory);
  app.post('/deleteAjaxSubCategory', deleteSubCategory);
}
